...
Shader "Custom/Iridescent" {
    Properties {
        ...
        [MaterialToggle] _LocalSpaceIridescence ("LocalSpaceIridescence", Float ) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform float _Cutoff;
            uniform fixed _LocalSpaceIridescence;
            ...
            float _GetRamp( float3 normal , float3 light , sampler2D anotherRamp , float anotherRampFull ){
            return GetRamp(normal, light, anotherRamp, anotherRampFull);
            }
...
////// Emissive:
                ...
                float lighting = (_GetRamp( normalDirection , lightDirection , _AnotherRamp , _AnotherRampFull )*attenuation);
                ...
                float3 OSIAxis = mul( unity_ObjectToWorld, float4(float3(_LocalVectorX,_LocalVectorY,_LocalVectorZ),0) ).xyz;
                float3 OSIBitangent = normalize((OSIAxis.rgb - (i.normalDir * dot(OSIAxis.rgb,i.normalDir)/dot(i.normalDir,i.normalDir))));
                float3 OSITangent = cross(i.normalDir,OSIBitangent);
                ...
                float2 iridescenceUV = (lerp(float2(dot(lerp(i.tangentDir,OSITangent,_LocalSpaceIridescence),perturbedView),dot(lerp(i.bitangentDir,OSIBitangent,_LocalSpaceIridescence),perturbedView)),(-1*mul( UNITY_MATRIX_V, float4(rotateView2( halfDirection , viewDirection , normalDirection ),0) ).xyz.rgb.rg),_ViewSpaceIridescence)*0.5+0.5);
