...
////// Emissive:
                ...
                float3 OSIBiangent = normalize((OSIAxis.rgb - (i.normalDir * dot(OSIAxis.rgb,i.normalDir)/dot(i.normalDir,i.normalDir))));
                float3 OSITangent = cross(i.normalDir,OSIBiangent);
                ...
                float2 iridescenceUV = (lerp(float2(dot(lerp(i.tangentDir,OSITangent,_LocalSpaceIridescence),perturbedView),dot(lerp(i.bitangentDir,OSIBiangent,_LocalSpaceIridescence),perturbedView)),(-1*mul( UNITY_MATRIX_V, float4(rotateView2( halfDirection , viewDirection , normalDirection ),0) ).xyz.rgb.rg),_ViewSpaceIridescence)*0.5+0.5);
