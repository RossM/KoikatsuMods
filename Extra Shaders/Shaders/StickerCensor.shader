    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float obzZ = ObjZ( float3(0,0,0) );
                float scale = lerp(length(viewOffset.rgb),(length(objScale)*0.3536),_ScaleMode);
                float2 matcapUV2 = saturate(((lerp(viewOffset.rgb,((obzZ/partZ)*viewOffset.rgb),_ScaleMode).rg/(scale*_Radius))*0.5+0.5));
                float4 _StickerTex_var = tex2D(_StickerTex,TRANSFORM_TEX(matcapUV2, _StickerTex));
                float hideZ = (obzZ-(scale*_HideDepth));
                float alpha = (_Color.a*_StickerTex_var.a*saturate(((lerp(GetObjectCenterDepth(),sceneZ,_AllowPartialHiding)-hideZ)/(lerp(hideZ,obzZ,_FadeInterval)-hideZ))));
