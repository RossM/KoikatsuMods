...
Shader "Custom/TranslucentBase" {
    ...
    SubShader {
        ...
        Pass {
            ...
            const float fresnelScale = 2.5;
            const float fresnelBase = -0.5;
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                clip(step(0.01,mainTexAlpha) - 0.5);
                float3 lightDirection = normalize(_WorldSpaceLightPos0.xyz);
                float3 halfDirection = normalize(viewDirection+lightDirection);
////// Lighting:
                float attenuation = LIGHT_ATTENUATION(i);
////// Emissive:
                float2 rampUV = float2((max(0,dot(normalDirection,lightDirection))*0.998+0.001),0.0);
                ...
                float3 rimRGB = GetRimLight( normalDirection , viewDirection , (saturate((_rimColor.rgb > 0.5 ?  (1.0-(1.0-2.0*(_rimColor.rgb-0.5))*(1.0-mainColorRGB)) : (2.0*_rimColor.rgb*mainColorRGB)) )*_rimColor.a) , _rimpower , _rimV , 0.0 , shadowRGB );
