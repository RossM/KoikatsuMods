    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 viewOffset = mul( unity_WorldToObject, float4((i.posWorld.rgb-objPos.rgb),0) ).xyz;
                float scale = lerp(length(viewOffset.rgb),(length(objScale)*0.2867),_ScaleMode);
                float2 stickerBase = ((-1*viewOffset.rgb.rg)/(scale*_Radius));
                float4 time = _Time;
                ...
                float alpha = (_Color.a*_StickerTex_var.a*saturate(((lerp(GetObjectCenterDepth(),max(0, LinearEyeDepth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sceneUVs)) - _ProjectionParams.g),_AllowPartialHiding)-min(hideZnear,hideZfar))/(max(hideZnear,hideZfar)-min(hideZnear,hideZfar)))));
