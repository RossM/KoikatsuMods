    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float2 noiseUV = ((float2(i.screenPos.x*(_ScreenParams.r/_ScreenParams.g), i.screenPos.y).rg*centerZ)+(float2(_AnimationU,_AnimationV)*t.g));
                float4 _NoiseTex_var = tex2D(_NoiseTex,TRANSFORM_TEX(noiseUV, _NoiseTex));
                float r = _Radius;
                float alpha = (_Color.a*(1.0 - GetPixelDistance( _GrabTexture , sceneUVs.rg , partZ , ((((_NoiseTex_var.r*2.0+-1.0)*_NoiseAmount)+1.0)*(r*0.1+0.0)*(1.0 / centerZ)) , _CameraDepthTexture )));
