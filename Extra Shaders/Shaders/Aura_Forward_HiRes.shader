...
Shader "Custom/Aura_Forward_HiRes" {
    ...
    SubShader {
        ...
        Pass {
            ...
            float GetPixelDistance( float2 uv , float radius ){
            return GetAuraDistance(uv, radius);
            }
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float r = _Radius;
                float2 gradientUV = saturate(float2(GetPixelDistance( sceneUVs.rg , ((((_NoiseTex_var.r*2.0+-1.0)*_NoiseAmount)+1.0)*(r*0.1+0.0)*(1.0 / centerZ)) ),0.0));
