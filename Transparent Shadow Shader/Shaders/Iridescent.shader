...
Shader "Custom/Iridescent" {
    Properties {
        ...
        _rimV ("rimV", Float ) = 0
        _rimColor ("rimColor", Color) = (0.602,0.715,0.742,1)
...
////// Emissive:
                ...
                float3 shadowColorRGB = lerp(float3(1,1,1),_ShadowColor.rgb,(shadowStrength*_shadowV));
                ...
                float brightness = min((1.0 - detailMaskG),lerp((_ShadowExtend*-0.5+0.0),1.0,(_RampG_var.r*attenuation)));
                ...
                float3 rimRGB = saturate((_rimColor.rgb*(1.0 - (detailMaskG*(shadowStrength*0.5+1.0)))*_rimV*rimValue));
