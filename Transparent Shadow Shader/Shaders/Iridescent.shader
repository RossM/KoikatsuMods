...
Shader "Custom/Iridescent" {
    Properties {
        ...
        _IridescenceV ("IridescenceV", Float ) = 0
        _IridescenceRot ("IridescenceRot", 2D) = "gray" {}
        _IridescenceMask ("IridescenceMask", 2D) = "white" {}
        [MaterialToggle] _ViewSpaceIridescence ("ViewSpaceIridescence", Float ) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            // Compute the rotation of "view" around the axis that takes "from" to "to". "from" and "to" must be normalized.
            float3 axis = cross(from, to); // The normalized axis of rotation * sin(r)
            float sinr2 = dot(axis, axis); // sin(r)^2
            float cosr = dot(from, to); // cos(r)
            return cosr * view + cross(axis, view) + (1 - cosr) * axis * dot(axis, view) / sinr2;
            }
            
            uniform fixed _ViewSpaceIridescence;
            float3 rotateView2( float3 from , float3 to , float3 view ){
            return rotateView(from, to, view);
            }
