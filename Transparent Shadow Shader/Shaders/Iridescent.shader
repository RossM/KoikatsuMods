...
Shader "Custom/Iridescent" {
    Properties {
        ...
        _rimV ("rimV", Float ) = 0
        _rimColor ("rimColor", Color) = (0.502,0.564,0.576,1)
        _rimX ("rimX", Float ) = -0.4
        _rimY ("rimY", Float ) = 0.29
...
////// Emissive:
                ...
                float iridescenceFactor = (pow(0.5*dot(normalDirection,halfDirection)+0.5,(_IridescencePower+1.0))*brightness*_IridescenceAlpha);
                ...
                float rimValue = saturate((pow((1.0 - max(0,dot(normalDirection,viewDirection))),(_rimpower*9.0+1.0))*5.882353+-1.941177));
                float node_1613 = 0.0;
                float3 rimRGB = saturate(lerp(_rimColor.rgb,float3(1,1,1),(_rimX + ( (((1.0 - (detailMaskG*(shadowStrength*-0.5+1.5)))*_rimV*rimValue) - node_1613) * (_rimY - _rimX) ) / (1.0 - node_1613))));
                float3 emissive = ((textureRGB*_LightColor0.rgb*lerp(shadowColorRGB,float3(1,1,1),brightness))+glossRGB+rimRGB);
