...
Shader "Custom/Iridescent" {
    ...
    SubShader {
        ...
        Pass {
            ...
            float sinr2 = dot(axis, axis);
            float cosr = dot(from, to);
...
////// Emissive:
                ...
                float textureRelativeSpecular_ang = (_IridescenceRot_var.g*6.283186+-3.141593);
                float textureRelativeSpecular_spd = 1.0;
                float textureRelativeSpecular_cos = cos(textureRelativeSpecular_spd*textureRelativeSpecular_ang);
                float textureRelativeSpecular_sin = sin(textureRelativeSpecular_spd*textureRelativeSpecular_ang);
                float2 textureRelativeSpecular_piv = float2(0,0);
                float3 perturbedView = rotateView( normalDirection , i.normalDir , halfDirection ); // This function computes a rotation around an axis that takes the from vector to the to vector. We use it to change the half direction from perturbed normal space to unperturbed normal space before finding the iridescence UV coordinates.
                float2 textureRelativeSpecular = (mul(float2(dot(i.tangentDir,perturbedView),dot(perturbedView,i.bitangentDir))-textureRelativeSpecular_piv,float2x2( textureRelativeSpecular_cos, -textureRelativeSpecular_sin, textureRelativeSpecular_sin, textureRelativeSpecular_cos))+textureRelativeSpecular_piv);
                float2 iridescenceUV = (textureRelativeSpecular*0.5+0.5);
                ...
                float3 iridescenceFactor = (pow(0.5*dot(normalDirection,halfDirection)+0.5,(_IridescencePower*9.0+1.0))*brightness*_IridescenceV*_IridescenceMask_var.rgb);
