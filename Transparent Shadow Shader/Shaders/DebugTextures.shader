...
////// Emissive:
                ...
                float3 viewSpaceReflection = lerp(mul( UNITY_MATRIX_V, float4(viewReflectDirection,0) ).xyz.rgb,reflect(ViewSpacePosition( i.posWorld.rgb ),viewNormal),_MatcapMode);
                float2 matcapUV = saturate((lerp((viewSpaceReflection.rg/length((viewSpaceReflection+float3(0,0,1)))),normalize(viewNormal.rg),step(viewNormal.b,0.0))*0.5+0.5));
